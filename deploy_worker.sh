...
DOCKER_APP_NAME=mymap_worker

EXIST_BLUE=$(docker-compose -p ${DOCKER_APP_NAME}-blue -f worker_docker-compose.blue.yml ps | grep Up)

if [ -z "$EXIST_BLUE" ]; then
    echo "blue up"
    
    docker-compose -p ${DOCKER_APP_NAME}-blue -f worker_docker-compose.blue.yml up -d
    ...
    docker-compose -p ${DOCKER_APP_NAME}-green -f worker_docker-compose.green.yml up -d
